...
router.get("/11", function (req, res, next) {
  try {
    ...
    Order.aggregate()
      ...
      .unwind("customer")
      .match({
        "customer.paymentType": new RegExp(paymentType),
      })
      .project({
        customerId: 0,
      })
...
//QUESTION 17
router.get("/17", async function (req, res, next) {
  try {
...
//QUESTION 18
router.get("/18", async function (req, res, next) {
  try {
...
//QUESTION 19
router.get("/19", async function (req, res, next) {
  try {
    let response = await Supplier.aggregate()
      .lookup({
        from: "products",
        localField: "_id",
        foreignField: "suppliersId",
        as: "products",
      })
      .unwind("products")
      .group({
...
//QuesTION23
router.get("/23", async function (req, res, next) {
  try {
...
//question 28
router.get("/28", async function (req, res, next) {
  try {
    let { fromDate, toDate } = req.query;
    fromDate = new Date(fromDate);
    const tmpToDate = new Date(toDate);
...
//QUESTION 32
router.get("/32", async function (req, res, next) {
  try {
    let { fromDate, toDate } = req.query;
    fromDate = new Date(fromDate);
    const tmpToDate = new Date(toDate);
...
//QUESTION 34
router.get("/34", async function (req, res, next) {
  try {
    let { fromDate, toDate } = req.query;
    fromDate = new Date(fromDate);
    const tmpToDate = new Date(toDate);
